...
class M_Admin extends CI_Model
{
  ...
  public function buat_idanggotaotomatis($table_name,$kodeawal,$idkode,$orderbylimit)
  {
      $query = $this->db->query("select * from $table_name $orderbylimit"); // cek dulu apakah ada sudah ada kode di tabel.
        ...
        $nomor = $query->num_rows();
        $kode = $cd + 1;
        $kodejadi = $kodeawal.$pemisah.date("Y").$pemisah.$kode;    // hasilnya CUS-0001 dst.
        $kdj = $kodejadi;
...
function filterbytanggal($tanggalawal,$tanggalakhir){
    $query = $this -> db ->query ("SELECT * From tb_pinjam join tb_user ON tb_user.id_login=tb_pinjam.id_login where tgl_pinjam BETWEEN '$tanggalawal' and '$tanggalakhir' ORDER BY tgl_pinjam ASC ");
    return $query-> result();
}
