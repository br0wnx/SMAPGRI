...
class Login extends CI_Controller {
	...
	{
		$this->data['title_web'] = 'Sistem Informasi Perpustakaan SMAPI';
    ...
    public function auth()
    {
        ...
        // auth
        $proses_login = $this->db->query("SELECT * FROM tb_user WHERE user='$user' AND pass = md5('$pass')");
    //    $proses_login = $this->db->query("SELECT * FROM tb_user WHERE user='$user'");
        ...
        if($row > 0)
        {
            ...
            $this->session->set_userdata('ses_id',$hasil_login['id_login']);
             $this->session->set_userdata('id_login',$hasil_login['id_login']);

            echo '<script>window.location="'.base_url().'dashboard";</script>';
        }else{

            echo '<script>alert("Username dan Password yang anda masukkan salah");
