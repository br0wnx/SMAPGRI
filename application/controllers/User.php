...
class User extends CI_Controller {
    ...
    {	
		...
		$this->data['laporan_pengguna'] = $this->M_Admin->get_table('tb_user');
    ...
    {	
        $this->data['idbo'] = $this->session->userdata('ses_id');
        $this->data['user'] = $this->M_Admin->get_table('tb_user');
// dibawah ini untuk tambah id anggota secara otomatis
        $this->data['id_anggota'] = $this->M_Admin->buat_idanggotaotomatis('tb_user','AG','id_login','ORDER BY id_login DESC LIMIT 1'); 
// end untuk tambah id anggota secara otomatis       
    ...
    public function add()
    {
        ...
        $nisnip = htmlentities($this->input->post('nisnip',TRUE));
		...
		$dd = $this->db->query("SELECT * FROM tb_user WHERE user = '$user' OR email = '$email'");
		if($dd->num_rows() > 0)
		{
    ...
    {	
		if($this->session->userdata('level') == 'Petugas'){
			...
			$this->data['idbo'] = $this->session->userdata('ses_id');
			$count = $this->M_Admin->CountTableId('tb_user','id_login',$this->uri->segment('3'));
			if($count > 0)
			{			
				$this->data['user'] = $this->M_Admin->get_tableid_edit('tb_user','id_login',$this->uri->segment('3'));
			}else{
			...
			if($count > 0)
			{			
				$this->data['user'] = $this->M_Admin->get_tableid_edit('tb_user','id_login',$this->session->userdata('ses_id'));
			}else{
				echo '<script>alert("USER TIDAK DITEMUKAN");window.location="'.base_url('user').'"</script>';
			}
		}
    ...
    {	
		if($this->session->userdata('level') == 'Petugas'){
			...
			$this->data['idbo'] = $this->session->userdata('ses_id');
			$count = $this->M_Admin->CountTableId('tb_user','id_login',$this->session->userdata('ses_id'));
			if($count > 0)
			{			
				$this->data['user'] = $this->M_Admin->get_tableid_edit('tb_user','id_login',$this->session->userdata('ses_id'));
			}else{
    ...
    public function upd()
    {
		...
		if(!$this->upload->do_upload('gambar'))
		{
			if($this->input->post('pass') !== ''){
				...
				$this->M_Admin->update_table('tb_user','id_login',$id_login,$data);
				if($this->session->userdata('level') == 'Petugas')
				{
    ...
    public function del()
    {
        ...
        $user = $this->M_Admin->get_tableid_edit('tb_user','id_login',$this->uri->segment('3'));
        unlink('./assets_style/image/'.$user->foto);
		$this->M_Admin->delete_table('tb_user','id_login',$this->uri->segment('3'));
